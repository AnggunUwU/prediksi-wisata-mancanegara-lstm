...
# ======================================
# 4. Training Model - Perbaikan Split Timeline
# ======================================
# Hitung split point berdasarkan timeline
...
try:
    ...
    # Hitung indeks yang benar untuk plotting
    train_start_idx = time_steps
    train_end_idx = train_start_idx + len(X_train)
    test_end_idx = train_end_idx + len(X_test)
    
    # Dapatkan tanggal yang sesuai
    train_dates = df_filtered['Tahun-Bulan'].iloc[train_start_idx:train_end_idx]
    test_dates = df_filtered['Tahun-Bulan'].iloc[train_end_idx:test_end_idx]
    ...
    if len(test_dates) > 0:
        ax1.axvline(x=test_dates.iloc[0], color='gray', linestyle=':', label='Pemisah Train-Test')
